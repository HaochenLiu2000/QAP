...
for epoch in range(num_epochs):
    ...
    print(f"Epoch {epoch + 1}/{num_epochs}, Loss_train: {train_loss}, acc_dev: {correct2}/511")
    if correct2>=acc_test:
        torch.save(decoder.state_dict(), 'model.pth')
        acc_test=correct2
    with open(train_note, 'a') as f:
        f.write(f"Epoch {epoch + 1}/{num_epochs}, Loss_train: {train_loss}, acc_dev: {correct2}/511")
        f.write('\n')